...
// Optional constructor: accepts double
Rational::Rational(double d) {
    if (std::isnan(d)) {
        ...
        // Concept: Math with primitive types
        // To turn a double into a fraction, we can multiply by a power of 10
        // until the fractional part is gone, then simplify.
        double intPart;
        double fracPart = std::modf(std::abs(d), &intPart);
        int sign = (d < 0) ? -1 : 1;
        
        int denom = 1;
        // Keep multiplying by 10 until the fraction part is extremely small
...
std::string Rational::str() const {
    ...
    // Normal case using a string stream to combine text and numbers
...
// Concept: Modifiers (Setters)
// Replaces the current value with newly provided numbers
...
// Parses a string (like "(56/77)" or "-34/2") and overwrites old values
void Rational::str(std::string s) {
    ...
    // Eliminate brackets if present
    if (s.front() == '(' && s.back() == ')') {
        s = s.substr(1, s.length() - 2);
    }
    
    // Concept: String Streams (similar to cin, but for strings)
    std::istringstream iss(s);
    char slash;
    int numerator, denominator;
    
    // Try to extract: Number -> '/' Character -> Number
    if (iss >> numerator >> slash >> denominator && slash == '/') {
        num_ = numerator;
        den_ = denominator;
    } else {
        // If we didn't find a slash, maybe it's just a regular integer
        iss.clear(); // Reset error flags from the previous failing read
        iss.str(s);  // Reload string from the start
        if (iss >> numerator) {
            num_ = numerator;
            den_ = 1;
        } else {
...
Rational Rational::add(Rational other) const {
    ...
    // Using long long prevents numbers from overflowing during multiplication
...
Rational Rational::mul(Rational other) const {
    // Normal math nicely handles X/0 * 0/X scenarios when generating nans
    // but doing explicit catch avoids unexpected division overlaps
    if (is_nan() || other.is_nan()) return Rational(0, 0);
...
Rational Rational::pow(int n) const {
    ...
    // Negative exponent concept: x^-y is the same as (1/x)^y
    if (p < 0) {
        p = -p;
...
// Square root: sqrt(a/b) = sqrt(a) / sqrt(b)
Rational Rational::sqrt() const {
    // If number is negative, inf or nan, roots are invalid
    if (is_neg() || is_inf() || is_nan()) {
        return Rational(0, 0); 
    }
    
    // A primitive check for perfect squares
    int n_root = std::round(std::sqrt(num_));
    int d_root = std::round(std::sqrt(den_));
    
    // Concept: Perfect Root Verification
    // Roots of rationals must themselves be integers
    if (n_root * n_root == num_ && d_root * d_root == den_) {
        return Rational(n_root, d_root);
    }
    
    // Return NaN if they're not perfect integer squares
    return Rational(0, 0); 
}